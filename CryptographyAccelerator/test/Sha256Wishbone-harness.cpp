...
#endif
#include <iostream>
class Sha256Wishbone_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    Sha256Wishbone_api_t(VSha256Wishbone* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_bus_data_wr)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_bus_addr)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_bus_sel)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_bus_we)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_bus_stb)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_bus_cyc)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_bus_data_rd)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_bus_err)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_bus_ack)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["Sha256Wishbone.reset"] = 0;
    }
#if VM_TRACE
...
static Sha256Wishbone_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VSha256Wishbone* top = new VSha256Wishbone;
    std::string vcdfile = "test_build/Sha256Wishbone.vcd";
